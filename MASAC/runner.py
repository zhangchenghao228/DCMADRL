...
from maddpg.maddpg import CRITIC_NET
...
class Runner:
    ...
    def run_tensor(self):
        ...
        for episode in tqdm(range(self.args.max_episode)):
            ...
            agent_reward_record = [0] * self.n_agents
            agent_cost = [0] * self.n_agents
            ...
            while not done_flag:
                ...
                s_next, r, done_flag, week, hour, PPD, r_local, r_global, cost, cost_violate_sum, cost_violate_count, reward_record = self.env.step(actions)

                self.buffer.store_episode(s[:self.n_agents], u, r[:self.n_agents], s_next[:self.n_agents])
                s = s_next
                reward = np.array(r)
                ...
                for i in range(self.n_agents):
                    agent_cost[i] += cost[i]
                for i in range(self.n_agents):
                    agent_reward_record[i] += reward_record[i]
                ...
                agent_total_cost[0] += np.sum(cost)
                ...
                if (time_step + 1) % 4 == 0 and self.buffer.current_size >= 256:
                    for agent_id, agent in enumerate(self.agents):
                        ...
                        id = agent_id
                        agent.learn(transitions, other_agents, self.critic_net[id], logger)
                end_time = time.time()
                print('--------------------------------------------------------')
            ...
            hist_reward_record.append(agent_reward_record)
            hist_cost.append(agent_cost)
            hist_violate_sum.append(agent_cost_violate_sum)
            ...
            total_reward = agent_reward_record[0] + agent_reward_record[1] + agent_reward_record[2] + agent_reward_record[3] + agent_reward_record[4]
            total_cost = agent_cost[0] + agent_cost[1] + agent_cost[2] + agent_cost[3] + agent_cost[4]
            ...
            to_log1 = {
                ...
                "agent_reward_1": agent_reward_record[0],
                "agent_reward_2": agent_reward_record[1],
                "agent_reward_3": agent_reward_record[2],
                "agent_reward_4": agent_reward_record[3],
                "agent_reward_5": agent_reward_record[4],
                "agent_cost_1": agent_cost[0],
                "agent_cost_2": agent_cost[1],
                "agent_cost_3": agent_cost[2],
                "agent_cost_4": agent_cost[3],
                "agent_cost_5": agent_cost[4],
            ...
            if episode % 10 == 0:
                ...
                df5 = pd.DataFrame(hist_reward_record)
                df6 = pd.DataFrame(hist_cost)
                df7 = pd.DataFrame(hist_violate_sum)
                df8 = pd.DataFrame(hist_violate_count)

                df9 = pd.DataFrame(hist_total_cost)
                df10 = pd.DataFrame(hist_total_cost_violate_sum_average)
                df11 = pd.DataFrame(hist_total_cost_count_sum_probability)
                df12 = pd.DataFrame(hist_total_violate_count)
                ...
                df5.to_excel('reward_record.xlsx', index=False)
                df6.to_excel('cost.xlsx', index=False)
                df7.to_excel('hist_violate_sum.xlsx', index=False)
                df8.to_excel('hist_violate_count.xlsx', index=False)

                df9.to_excel('total_cost.xlsx', index=False)
                df10.to_excel('hist_total_cost_violate_sum_average.xlsx', index=False)
                df11.to_excel('hist_total_cost_count_sum_probability.xlsx', index=False)
                df12.to_excel('total_violate_count.xlsx', index=False)
