...
class MADDPG:
    ...
    def train_new(self, transitions, other_agents, critic_net, logger=None, n_agents=5):
        ...
        with torch.no_grad():
            ...
            for agent_id in range(n_agents):
                if agent_id == self.agent_id:
                    ...
                    entroy_temp = -batch_u_next_log_prob
                    u_next.append(batch_u_next)
                    entropy.append(entroy_temp)
                else:
                    batch_u_next = other_agents[index].policy.actor_network(o_next[agent_id])[0]
                    u_next.append(batch_u_next)
                    index += 1
        ...
        critic_net.upda(o_aggregation0, u_aggregation0, o_aggregation_next0, u_aggregation_next1, r, entropy[0], self.log_alpha, logger)
        ...
        actor_loss = critic_net.actor_loss(o_aggregation0, u_aggregation1, entropy_actor, self.log_alpha)
