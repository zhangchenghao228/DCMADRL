...
class EnergyPlusEnvironment:
    def __init__(self) -> None:
        ...
        self.swanlab = swanlab.init(
            project="safe_RL_cost",
            workspace="ZCH_SDU",
            tags=["MASAC", "We=0.6", "Wc=0.4"],
            # 跟踪超参数和运行元数据
            config={
                "architecture": "MASAC",
            },
            experiment_name = "MASAC",
            name = "MASAC"
        )
        # self.cfg = cfg#参数配置文件，里面是相关参数
        self.episode = -1
        self.timestep = 0
        self.obs_copy = {}
    ...
    def step(self, action):
        ...
        reward, reward_local, reward_global, cost, cost_sum, cost_count, reward_record = self.get_reward  # 这是一个标量
        to_log = {
        ...
        return obs_test, reward, done, self.week, self.day_hour, PPD, reward_local, reward_global, cost, cost_sum, cost_count, reward_record
    ...
    def get_reward(self):
        ...
        energy = (obs["elec_hvac"] + obs["elec_heat"]) / 3600000 / 20  # 将电能消耗量从瓦特秒转换为千瓦时,这个值可能是在1以下  ########30，这个地方原来是除以20，现在我除以40试一试
        energy_record = (obs["elec_hvac"] + obs["elec_heat"]) / 20000000 / 2.5 # 将电能消耗量从瓦特秒转换为千瓦时,这个值可能是在1以下,应该是要调节到同一个数量级
        for o, c, c_voliate, cost_sum_a, cost_count_a in zip(occups_vals, c_result, cost_violate, cost_PPD_violate_sum, cost_PPD_count_sum):
            if o == 0:
                r = -w_e * energy
                reward_local_temp = 0
                ...
                reward_global_temp = -w_e * energy
                c_voliate_temp = cof * c_voliate
                cost_sum_temp = 100 * cost_sum_a
                cost_count_temp = cost_count_a
            reward.append(r)
            reward_local.append(reward_local_temp)
            reward_global.append(reward_global_temp)
            cost.append(c_voliate_temp)  # 存放每个agent的奖励
            cost_sum.append(cost_sum_temp)
            cost_count.append(cost_count_temp)
            reward_record.append(- energy_record)
            # cost.append(c_record)
        return reward, reward_local, reward_global, cost, cost_sum, cost_count, reward_record
