...
class MADDPG:
    ...
    def train_new(self, transitions, other_agents, critic_net, cost_critic, logger=None, n_agents=5):
        ...
        cost_critic_loss = cost_critic.actor_loss(o, u)


        actor_loss = actor_critic_loss + self.lagrangian * cost_critic_loss
        ...
        alpha_loss = torch.mean(
            (entropy_actor - self.target_entropy).detach() * self.log_alpha.exp())
        self.log_alpha_optimizer.zero_grad()
        alpha_loss.backward()
        self.log_alpha_optimizer.step()

        # ---------- Lagrangian 更新 ----------
        cost_mean = cost_critic_loss.detach().mean()
        lagrangian_loss = (-self.lagrangian * (cost_mean - self.cost_limit)).mean()
...
class COST_CRITIC_NET:
    ...
    def actor_loss(self, o, u):
        q1_value = self.critic_network1(o, u)
        # q2_value = self.critic_network2(o, u)
        # actor_loss = torch.mean(torch.min(q1_value, q2_value))
        actor_loss = torch.mean(q1_value)
        return actor_loss
