...
class MADDPG:
    ...
    def train_new(self, transitions, critic_net, cost_net, logger=None):
        ...
        with torch.no_grad():  # adv and v_target have no gradient
            ...
            Jc = (((cost_mean - self.cost_limit) * (1 - self.args.gamma)) +  adv_cost.mean()) / (adv_cost.std() + 1e-8) # 计算当前的cost偏差,这是一个标量
        ...
        # Jc = (cost_mean - self.cost_limit) * (1 - self.args.gamma)  # 计算当前的cost偏差,这是一个标量
        ...
        for _ in range(self.K_epochs): 
            ...
            for index in indexes:
                ...
                actor_loss_cost = self.kappa * F.relu(surr_cadv + Jc)
