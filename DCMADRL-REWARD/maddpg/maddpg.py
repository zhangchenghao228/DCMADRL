import torch
import os
from maddpg.actor_critic import Actor, Critic, Cost_critic, QuantileMlp, QuantileMlp_cost
...
class MADDPG:
    ...
    def train_new(self, transitions, other_agents, critic_net, cost_critic, logger=None, n_agents=5):
        ...
        # o_next.append(transitions['o_next_%d' % self.agent_id])  # o现在
        # u_next = []
        ...
        #critic网络更新
        critic_net.upda(o, u, o_next, u_next, r, entropy, self.log_alpha, next_tau_hat, next_presum_tau, logger)

        #cost网络更新
        cost_critic.upda([o[self.agent_id]], [u[self.agent_id]], [o_next[self.agent_id]], [u_next[self.agent_id]], c, next_tau_hat, next_presum_tau, logger)
        ...
        actor_critic_loss = critic_net.actor_loss(o, u, entropy_actor, self.log_alpha, new_tau_hat, new_presum_tau)


        self.actor_loss = actor_critic_loss

        cost_critic_loss = cost_critic.actor_loss([o[self.agent_id]], [u[self.agent_id]], self.cost_limit, new_tau_hat, new_presum_tau)
...
class COST_CRITIC_NET:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        self.critic_network1 = QuantileMlp_cost().to(device)
        self.critic_target_network1 = QuantileMlp_cost().to(device)
        self.critic_target_network1.load_state_dict(self.critic_network1.state_dict())
        self.critic_optim1 = torch.optim.Adam(self.critic_network1.parameters(), lr=self.args.lr_critic)
...
class Quantile_net:
    ...
    def upda(self, o, u, o_next, u_next, r, entropy, log_alpha, next_tau_hat, next_presum_tau, logger):
        with torch.no_grad():
            ...
            q_next_2 = self.critic_target_network2(o_next, u_next, next_tau_hat).detach()
            next_value = torch.min(q_next_1, q_next_2) + log_alpha.exp() * entropy
        ...
        z2_pred = self.critic_network2(o, u, tau_hat)
        zf1_loss = self.quantile_regression_loss(z1_pred, z_target, tau_hat, next_presum_tau)#next_presum_tau是target distribution fraction tau_i+1 - tau_i
    ...
    def actor_loss(self, o, u, entropy, log_alpha, new_tau_hat, new_presum_tau):
        ...
        z2_new_actions = self.critic_network2(o, u, new_tau_hat)
