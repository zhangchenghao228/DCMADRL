...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="DDPG_SAC",
...
class EnergyPlusEnvironment:
    def __init__(self) -> None:
        ...
        self.swanlab = swanlab.init(
            ...
            workspace="ZCH_SDU",
            tags=["MASAC", "集中式reward critic", "集中式cost critic", "代价函数为超出的PPD值"],
            experiment_name =  "MASAC_IPO_0.25_reward_Distribution",  # 添加实验名字
            # 跟踪超参数和运行元数据
            config={
                "architecture": "MASAC_IPO_distribution_reward and cost critic network is centralized",
                # "name": "MASAC_IPO_0.25_reward_Distribution",
                "notes": "MASAC_IPO_0.25_reward_Distribution",
            },
        )
        self.episode = -1
        self.timestep = 0
    ...
    def step(self, action):
        ...
        PPD = np.array(
            [obs["PPD_1"] / 100, obs["PPD_2"] / 100, obs["PPD_3"] / 100, obs["PPD_4"] / 100, obs["PPD_5"] / 100])
        return obs_test, reward, reward_local, reward_global, done, PPD, cost_violate_sum, cost_count
    ...
    def get_reward(self):
        ...
        # for PPD_temp in PPD_vals:
        #     if PPD_temp > PPD_thres:
        #         cost_PPD_violate_sum.append(PPD_temp - PPD_thres)
        #     else:
        #         cost_PPD_violate_sum.append(0)
        '''这个值对于5个智能体都是一样的，是不是应该寻找一个替代值'''
        # TODO find a good function to evaluate the temperature reward
        energy = (obs["elec_hvac"] + obs["elec_heat"]) / 20000000 / 2.5# 将电能消耗量从瓦特秒转换为千瓦时,这个值可能是在1以下,应该是要调节到同一个数量级
        for o, c, cost_PPD_violate, cost_PPD_count in zip(occups_vals, c_result, cost_PPD_violate_sum, cost_PPD_count_sum):
            if o == 0:
                ...
                reward_global_temp = - energy
                cost_PPD_violate_temp = 100 * cost_PPD_violate
